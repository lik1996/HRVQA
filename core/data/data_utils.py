...
def proc_ans(ans, ans_to_ix):
    ...
    if ans['multiple_choice_answer'][-2:] == 'm2':
        value = float(ans['multiple_choice_answer'][:-2])
        if value > 0. and value <= 10.:
            ans_proc = "between 0m2 and 10m2"
            ans_  =  prep_ans(ans_proc)
            if ans_ in ans_to_ix:
                ans_score[ans_to_ix[ans_]] = 1.
        ...
        if value > 1000.:
            ...
            if ans_ in ans_to_ix:
                ans_score[ans_to_ix[ans_]] = 1.
    elif ans['multiple_choice_answer'] == 'fitnessorsoccer':
        ans_proc = 'fitness or soccer'
        ans_  =  prep_ans(ans_proc)
        if ans_ in ans_to_ix:
            ans_score[ans_to_ix[ans_]] = 1.
    elif ans['multiple_choice_answer'] == 'fitnessorbasketball':
        ans_proc ='fitness or basketball'
        ans_  =  prep_ans(ans_proc)
        if ans_ in ans_to_ix:
            ans_score[ans_to_ix[ans_]] = 1.
    elif ans['multiple_choice_answer'] == 'tennisorsoccer':
        ans_proc ='tennis or soccer'
        ans_  =  prep_ans(ans_proc)
        if ans_ in ans_to_ix:
            ans_score[ans_to_ix[ans_]] = 1.
    elif ans['multiple_choice_answer'] == 'basketballorsoccer':
        ans_proc ='basketball or soccer'
        ans_  =  prep_ans(ans_proc)
        if ans_ in ans_to_ix:
            ans_score[ans_to_ix[ans_]] = 1.
    else:
        ans_  =  prep_ans(ans['multiple_choice_answer'])
        if ans_ in ans_to_ix:
            ans_score[ans_to_ix[ans_]] = 1.
