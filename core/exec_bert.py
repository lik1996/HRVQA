...
from core.data.load_data_bert import DataSet
# from core.model.net import Net
# from core.model.mcan_ca_nips.net import Net as Net_ca
# from core.model.trra.net import Net as Net_trar
# from core.model.mca_aoa.net import Net as Net_mca_aoa
# from core.model.mca_gated.net import Net as Net_mca_ga
# from core.model.mca_fusion.net import Net as Net_mca_fu
from core.model.ours_v2.net import Net as Net_ours_v2
...
class Execution:
    ...
    def train(self, dataset, dataset_eval=None):
        ...
        # if self.__C.MODEL_METHOD == 'mca_ca':
        #     gen_funcs = {"softmax": torch.softmax,"sparsemax": partial(sparsemax, k=512),"tvmax": "tvmax"}
        #     gen_func = gen_funcs[self.__C.gen_func]
        #     net = Net_ca(self.__C,pretrained_emb,token_size,ans_size,gen_func=gen_func)
        # elif self.__C.MODEL_METHOD == 'mca':
        #     net = Net(self.__C, pretrained_emb, token_size, ans_size)
        # elif self.__C.MODEL_METHOD == 'trar':
        #     net = Net_trar(self.__C, pretrained_emb, token_size, ans_size)
        # elif self.__C.MODEL_METHOD == 'mca_fu':
        #     net = Net_mca_fu(self.__C, pretrained_emb, token_size, ans_size)
        # elif self.__C.MODEL_METHOD == 'mca_aoa':
        #     net = Net_mca_aoa(self.__C, pretrained_emb, token_size, ans_size)
        # elif self.__C.MODEL_METHOD == 'mca_ga':
        #     net = Net_mca_ga(self.__C, pretrained_emb, token_size, ans_size)
        # elif self.__C.MODEL_METHOD == 'oursv2':
        #     net = Net_ours_v2(self.__C, ans_size)
        net = Net_ours_v2(self.__C, ans_size)
        ...
        if self.__C.MODEL_METHOD == 'trar' or self.__C.MODEL_METHOD == 'mca_aoa' or self.__C.MODEL_METHOD == 'mca_ga':
        ...
        for epoch in range(start_epoch, self.__C.MAX_EPOCH):
            ...
            for step, (
                ...
                for accu_step in range(self.__C.GRAD_ACCU_STEPS):
                    ...
                    sub_ques_idx = ques_idx[accu_step * self.__C.SUB_BATCH_SIZE: (accu_step + 1) * self.__C.SUB_BATCH_SIZE]
                    sub_attention_mask = attention_mask[accu_step * self.__C.SUB_BATCH_SIZE: (accu_step + 1) * self.__C.SUB_BATCH_SIZE]
                    sub_token_type_ids = token_type_ids[accu_step * self.__C.SUB_BATCH_SIZE: (accu_step + 1) * self.__C.SUB_BATCH_SIZE]
                    # sub_ques_ix_iter = \
                    ...
                    if self.__C.VERBOSE:
                        ...
                        print("\r[version %s][method %s][epoch %2d][step %4d/%4d][%s] loss: %.4f, lr: %.2e" % (
                            self.__C.VERSION,
                            self.__C.MODEL_METHOD,
                            epoch + 1,
                            step,
    ...
    def eval(self, dataset, state_dict=None, valid=False):
        ...
        if valid:
            ...
            if self.__C.CKPT_PATH is not None:
                result_eval_file = \
                    ...
                    'result_run_' + self.__C.CKPT_VERSION + self.__C.MODEL_METHOD + \
                    '.json'
            else:
                result_eval_file = \
                    self.__C.RESULT_PATH + \
                    'result_run_' + self.__C.CKPT_VERSION + \
                    '_epoch' + str(self.__C.CKPT_EPOCH) + self.__C.MODEL_METHOD + \
                    '.json'
        ...
        if valid:
            ...
            for quesType in vqaEval.accuracy['perQuestionType']:
                logfile.write("%s : %.02f\t" % (quesType, vqaEval.accuracy['perQuestionType'][quesType]))
            logfile.write("\n\n")
            logfile.close()
